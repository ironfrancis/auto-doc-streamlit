...
def get_template_files(template_dir: str = "app/html_templates") -> List[str]:
    """获取HTML模板文件列表"""
    try:
...
def restore_data(backup_path: str) -> Dict:
    ...
    try:
        ...
        # 恢复频道数据
        backup_channels = os.path.join(backup_dir, "channels_new.json")
        if os.path.exists(backup_channels):
            ...
            with open(backup_channels, 'r', encoding='utf-8') as src, \
                ...
                dst.write(src.read())
            restored_files.append("channels_new.json")
