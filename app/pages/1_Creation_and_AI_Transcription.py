...
CHANNELS_PATH = get_json_data_dir() / "channels_new.json"
...
with sel_col2:
    ...
    selected_endpoint = st.selectbox("选择LLM端点", endpoint_names, index=endpoint_index, key="endpoint_selector") if endpoint_names else ""
...
if st.button(get_text("transcribe_btn")):
    if not (md_input.strip() or text_input.strip() or link_input.strip()):
        ...
        # 添加提示词块内容（任务要求）
        if channel_obj and channel_obj.get("selected_blocks"):
            try:
                from utils.prompt_block_management import PromptBlockManager
                prompt_manager = PromptBlockManager()
                
                # 获取选中的提示词块
                selected_blocks = channel_obj.get("selected_blocks", [])
                if selected_blocks:
                    prompt_parts.append("# 任务要求")
                    
                    # 直接显示每个提示词块的内容
                    for block_id in selected_blocks:
                        block = prompt_manager.get_block_by_id(block_id)
                        if block:
                            prompt_parts.append(f"**{block['name']}:** {block.get('content', '无内容')}")
                
                # 添加自定义块
                custom_blocks = channel_obj.get("custom_blocks", {})
                if custom_blocks:
                    for key, block in custom_blocks.items():
                        prompt_parts.append(f"**{block['name']}:** {block['content']}")
                        
            except ImportError:
                st.warning("⚠️ 无法加载提示词块，将使用基础提示词")
...
def get_all_md_files():
    ...
    # 方法2: 如果方法1失败，尝试从工作目录查找
    if not project_root:
        cwd = os.getcwd()
        if 'Auto-doc-streamlit' in cwd:
            auto_doc_index = cwd.find('Auto-doc-streamlit')
            project_root = cwd[:auto_doc_index + len('Auto-doc-streamlit')]
    
    # 方法3: 最后备用 - 硬编码路径
    if not project_root or not os.path.exists(os.path.join(project_root, 'workspace')):
        # 从当前文件位置计算相对路径
        project_root = os.path.join(current_file_dir, '..', '..')
        project_root = os.path.abspath(project_root)
    
    # 从workspace目录读取
    workspace_md_dir = os.path.join(project_root, "workspace", "articles", "md_review")
