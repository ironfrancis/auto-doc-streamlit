...
# 读取LLM端点（优先使用 API，失败则回退到文件）
endpoints = []
try:
    client = APIClient()
    endpoints_data = client.get_llm_endpoints()
    # 转换 API 格式到页面期望的格式
    for ep in endpoints_data:
        converted_ep = client.convert_llm_endpoint_to_legacy_format(ep)
        endpoints.append(converted_ep)
except (APIError, APIConnectionError):
    # API 不可用，回退到文件读取
