...
def get_channels_file():
    """获取频道配置文件路径"""
    # 使用简化路径管理
    from scripts.utils.simple_paths import CONFIG_DIR
...
def load_llm_endpoints():
    """加载可用的LLM端点（优先使用 API，失败则回退到文件）"""
    # 尝试使用 API
    try:
        client = APIClient()
        endpoints = client.get_llm_endpoints()
        endpoint_names = [ep.get("name", f"端点{i+1}") for ep in endpoints]
        if endpoint_names:
...
def load_channels():
    ...
    try:
        ...
        converted_channels = []
        for ch in channels:
            converted_ch = client.convert_channel_to_legacy_format(ch)
            converted_channels.append(converted_ch)
...
def save_channels(channels_list):
    """保存频道列表（优先使用 API，失败则回退到文件）"""
    # 尝试使用 API 保存每个频道
    try:
        client = APIClient()
        # 获取现有频道列表
        existing_channels = client.get_channels()
        ...
        for channel in channels_list:
            ...
            if channel_id in existing_ids:
                # 更新现有频道
                try:
                    client.update_channel(channel_id, channel)
                except APIError as e:
                    st.warning(f"更新频道 {channel.get('name', channel_id)} 失败: {e}")
            else:
                # 创建新频道
                try:
                    client.create_channel(channel)
                except APIError as e:
                    st.warning(f"创建频道 {channel.get('name', channel_id)} 失败: {e}")
        
        # 删除不在列表中的频道
        current_ids = {ch.get("id") for ch in channels_list}
        for existing_ch in existing_channels:
            if existing_ch["id"] not in current_ids:
                try:
                    client.delete_channel(existing_ch["id"])
                except APIError as e:
                    st.warning(f"删除频道 {existing_ch.get('name', existing_ch['id'])} 失败: {e}")
        
        return True
    except (APIError, APIConnectionError) as e:
        # API 不可用，回退到文件保存
        st.warning(f"API 不可用，使用文件保存: {e}")
    except Exception as e:
        # 其他错误，也回退到文件保存
        st.warning(f"API 保存失败，使用文件保存: {e}")
