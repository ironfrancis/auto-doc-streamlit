...
# 使用简化路径管理
IMAGE_BEDS_PATH = os.path.join(CONFIG_DIR, "image_beds.json")
BED_TYPES = ["scdn", "lsky"]

# 读取已配置的图床
def load_image_beds():
    if os.path.exists(IMAGE_BEDS_PATH):
        try:
            with open(IMAGE_BEDS_PATH, "r", encoding="utf-8") as f:
                config = json.load(f)
                return config.get("image_beds", [])
        except json.JSONDecodeError as e:
            st.error(f"图床配置文件格式错误: {e}")
            return []
        except Exception as e:
            st.error(f"加载图床配置失败: {e}")
            return []
    else:
        st.error(f"图床配置文件不存在: {IMAGE_BEDS_PATH}")
        return []

def save_image_beds(image_beds):
    config = {
        "image_beds": image_beds,
        "metadata": {
            "version": "1.0",
    ...
    with open(IMAGE_BEDS_PATH, "w", encoding="utf-8") as f:
                    ...
                    with st.expander(bed['name'], expanded=False):
                        ...
                        with btn_cols[1]:
                            if st.button(T['zh']['delete'], key=f"del_bed_{card_idx}"):
                                image_beds.pop(card_idx)
                                save_image_beds(image_beds)
                                st.success(T['zh']['success'])
                                st.rerun()
                        with btn_cols[2]:
                            if st.button(T['zh']['default'], key=f"setdef_bed_{card_idx}"):
                                for i, bed2 in enumerate(image_beds):
                                    bed2["default"] = (i == card_idx)
                                save_image_beds(image_beds)
                    ...
                    with st.expander(T['zh']['new_bed'], expanded=False):
                        ...
                        if st.button(T['zh']['submit'], key=f"new_bed_submit_{card_idx}"):
                            if not name.strip():
                                ...
                                # 生成唯一ID
                                bed_id = f"{bed_type}_{len(image_beds)}"
